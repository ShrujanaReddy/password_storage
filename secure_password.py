...
def vaultScreen():
    ...
    btn = Button(window, text="+ Click to add a new website and password to your vault", command=addEntry)
    ...
    if (cursor.fetchall() != None):
        ...
        while True:
            ...
            btnnew = Label(window, text=(decrypt(array[i][3], encryptionKey)), font=("Helvetica", 12))
            #btnnew = Button(window, text="Copy password")#command = pyperclip.copy((decrypt(array[i][3], encryptionKey))))
            #btnnew = Button(window, text="Copy password", command=copy_password)
            ...
            def copy_password():
                ...
                button = tk.Button(root, text="Copy Password", command=copy_password)
                button.pack()

            btnnew.grid(column=2, row=(i+3))
